...
if($result = mysqli_query($conn, $sql)){
  if(mysqli_num_rows($result) != 0) {
    foreach($result as $post) {
      ...
      echo "<style>
      #wrapper {
        background: #eeeeee url('../cdn/community_banners/$cid.png');
...
echo '<section id="post-content" class="post post-subtype-default">  <header class="community-container">
  ...
  <div class="body">      <p class="post-content-text">' . htmlentities($body) . '</p>
          ';
...
if($result = mysqli_query($conn, $sql)){
  ...
  <div class="body">
    ...
    </div>    <p class="reply-content-text">' . htmlentities($body) . '</p>
