...
<?php
                    ...
                    if(isset($_POST["invite"])){
                      $invite = mysqli_escape_string($conn, $_POST["invite"]);
                      $sql = "SELECT * FROM group_chs WHERE invite = '$invite'";
                      if($result = mysqli_query($conn, $sql)){
                        if(mysqli_num_rows($result) != 0){
                            foreach($result as $ch){
                                $id = $ch["id"];
                                $list = explode(", ", $ch["member_list"]);
                                $list[] = $sessuid;
                                $list = implode(", ", $list);
                                $sql = "UPDATE group_chs SET member_list = '$list' WHERE id = '$id'";
...
<div class="main-column">
  <div class="post-list-outline">
    <h2 class="label">Use an GC invite</h2>
    <div id="guide" class="help-content">
      <div>
        <form method="post" enctype="multipart/form-data">
                    <p>GC invite: <input type="text" name="invite" required/></p>
                    <input type="submit" class="black-button" style="text-align: center; margin: 0 auto; display: block;" value="Add GC!" /> 
                    <?php
                    /* if(isset($_SESSION["id"])){
