...
if($result = mysqli_query($conn, $sql)){
  ...
  foreach($result as $user){
    ...
    $fav_post = htmlentities($user["fav_post"]);
    $karma = htmlentities($user["karma"]);
  ...
  if(isset($fav_post) && strlen($fav_post) != 0){
    echo '<a href="./post.php?id=' . $fav_post . '" id="sidebar-cover" style="background-image:url(./cdn/post_images/' . $fav_post . '.png">
    <img src="./cdn/post_images/' . $fav_post . '.png" class="sidebar-cover-image">
  </a>';
  }
    echo '<div id="sidebar-profile-body" class="">
  ...
  if(isset($sessuid)){
  if($id != $sessuid){
    $followlist = $user["follower_list"];
    $id_in_followlist = strpos($followlist, $sessuid);
    if($id_in_followlist === false){
      $followtext = "Follow";
    }
    else{
      $followtext = "Unfollow";
    }
    echo "<a href=./toggle_follow.php?id=$id class=button><p style=\"font-family: 'MiiverseSymbols'; display: inline; color: orange;\">E</p> $followtext</a>";
    $friendlist = $user["friend_list"];
    $friendrlist = $user["friend_request_list"];
    $sql = "SELECT * FROM accounts WHERE id = '$sessuid'";
    if($result = mysqli_query($conn, $sql)){
      foreach($result as $me){
        $mfriendrlist = $me["friend_request_list"];
        $id_in_mfriendrlist = strpos($friendrlist, $id);
      }
    }
    $id_in_friendlist = strpos($friendlist, $sessuid);
    $id_in_friendrlist = strpos($friendrlist, $sessuid);
    $flink = "./send_request.php?id=$id";
    if($id_in_friendlist === false && $id_in_friendrlist === false){
      $friendtext = "Friend";
    }
    elseif($id_in_friendlist === false && $id_in_friendrlist !== false){
      $friendtext = "Revoke friend request";
    } elseif($id_in_mfriendrlist !== false){
      $friendtext = "Accept request";
      $flink = "./accept_fr.php?id=$id";
    }
    else{
      $friendtext = "Unfriend";
    }    echo "<a href=$flink class=button><p style=\"font-family: 'MiiverseSymbols'; display: inline; color: orange;\">F</p> $friendtext</a>";
    if(in_array($id, $blocklistar)){
      $blocktext = "Unblock";
    } else {
      $blocktext = "Block";
    }
    echo "<a href=./toggleblock.php?id=" . $id . " class=button><p style=\"font-family: 'MiiverseSymbols'; display: inline; color: red;\">@</p> $blocktext</a>";
  }
}
  echo '<div class="sidebar-container sidebar-profile">
      <div class="profile-comment">
          <p class="js-full-text">' . $bio . '</p>
      </div>
    ...
    <div class="user-data">
      ...
      <div class="game-skill data-content">
        ...
        <div class="note">
          <span class="test-game-skill ' . $skill . '">' . ucfirst($skill) . '</span>
        </div>
      </div>
      
';
if(strlen($discord) != 0){
  echo '      <div class="favorite-game-genre">
...
echo '      <div class="game-skill data-content">
  <h4><span>Mii coins (karma)</span></h4>
  <div class="note">
    <span class="test-game-skill karma">' . $karma . '</span>
...
foreach($result as $posts){
  ...
  $nahlink = "\"./toggle_nah.php?id=$pid&r=user\"";
  ...
  echo '<div id="post" class="post post-subtype-default trigger" tabindex="0">  
  <p class="community-container">
      <a class="test-community-link" href="./community.php?id=' . $cid . '"><img src="./cdn/community_icons/' . $cid . '.png" class="community-icon">' . $cname . '</a></p>
    <a href="./user.php?id=' . $id . '" class="icon-container ' . $badge . '"><img src="' . "https://mii-unsecure.ariankordi.net/miis/image.png?data=" . $mii . "&type=face&width=120&expression=" . $feeling . "" . '" class="icon"></a>
  <p class="user-name"><a ' . $nickcss . '>' . $displayname . '</a></p>
    <p class="timestamp-container">
      <a class="timestamp" href="./post.php?id=' . $pid . '">' . $date . '</a>
    </p>  <div class="body">
    <div class="post-content">        <div class="tag-container">        </div>
            <p class="post-content-text" onclick="window.open(\'post.php?id=' . $pid . '\')">' . markdownToHTML(htmlentities($body)) . '</p>';
