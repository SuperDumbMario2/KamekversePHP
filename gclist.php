...
<div class="main-column">
  <div class="post-list-outline">
    <h2 class="label">Group Chats</h2>
    <div id="guide" class="help-content">
      <div>
      <div class="list news-list" tabindex="0">
      <a href="./create_gc.php" class="button">Create an group chat</a>
      <a href="./add_gc.php" class="button">Use an group chat invite</a>
        <?php
        $nickcolor = "#000";
        $sql = "SELECT * FROM group_chs";
        if($result = mysqli_query($conn, $sql)){
          foreach($result as $ch){
            $id = $ch["id"];
            $members = explode(", ", $ch["member_list"]);
            if(!in_array($sessuid, $members)){
              continue;
            }
            $name = $ch["name"];
            echo '<a href="./message.php?id=' . $id . '&mode=gm" class="icon-container"><img src="./cdn/gc_icons/' . $id . '.png" class="icon"></a>
            <a href="./message.php?id=' . $id . '&mode=gm"><div class="body">' . htmlentities($name) . '</div> </a>';
          }
        }
        ?>
        </div>
